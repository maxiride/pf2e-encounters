...
- `CreaturesTable` scrolls to browse the full result list instead of clicking through pages, and replaced the per-row `QBadge`/`QBtn`/`QTooltip` with plain HTML — those were mounting/unmounting on every scroll tick and were the actual cause of a multi-second scroll freeze. See [ADR 0005](docs/adr/0005-vue-pinned-to-3.4-for-table-virtual-scroll.md).
