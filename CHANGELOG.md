...
- `CreaturesTable` scrolls to browse the full result list (`q-table` `virtual-scroll`) instead of clicking through pages, and replaced the per-row `QBadge`/`QBtn`/`QTooltip` with plain HTML/CSS to keep each rendered row cheap.
